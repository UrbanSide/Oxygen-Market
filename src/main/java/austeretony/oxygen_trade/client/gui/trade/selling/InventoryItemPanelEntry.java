    ...
    public void initMarketData(ItemStackMarketData marketData) {   
        if (marketData != null) {
            if (marketData.getAveragePrice() > 0.0F)
                this.averageMarketPriceStr = OxygenUtils.formatDecimalCurrencyValue(TradeMenuScreen.DECIMAL_FORMAT.format(marketData.getAveragePrice()));
            ...
            this.marketDataTooltipStr = "(" + String.valueOf(marketData.getCompletedTransactionsAmount()) + 
                    ...
                    "): " + String.valueOf(TradeMenuScreen.DECIMAL_FORMAT.format(marketData.getAveragePrice()));
        }
    }
